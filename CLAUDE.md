...
`.github/workflows/ci.yml` runs six jobs on every push + PR:
...
3. **`rag-worker`** (ubuntu-latest) — same gates as `worker`, working-directory `apps/reyn-rag-worker` (269 tests, ~99% coverage).
4. **`lua`** (ubuntu-latest) — `apt install lua5.1` + `lua5.1 apps/reyn-bg3-mod/tests/lua/run.lua` (30 tests).
5. **`docs`** (ubuntu-latest) — lychee markdown link check + cspell (covers `docs/rag/**` too).
...
`.github/workflows/deploy-rag-worker.yml` is also `workflow_dispatch`-only. It applies kb-d1 migrations, pushes `OPENROUTER_API_KEY` + `KB_INGEST_KEY` secrets, then runs `wrangler deploy` for `apps/reyn-rag-worker`. Requires `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` + `OPENROUTER_API_KEY` + `KB_INGEST_KEY`. See `docs/rag/operations.md` for the one-time bootstrap prerequisite.
...
| RAG Worker | **269** (vitest-pool-workers) | ~99.3% / 96% (lines/branches) |
...
For the RAG worker, Vectorize and Workers AI have no local emulator; use `wrangler dev --remote` or the mock providers (default in tests). Full local-dev guide: [`docs/rag/operations.md`](docs/rag/operations.md).
...
- **RAG worker**: `apps/reyn-rag-worker/` — docs at `docs/rag/` (architecture, api, operations, sourcing, tuning)
