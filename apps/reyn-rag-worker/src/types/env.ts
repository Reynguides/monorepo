 ...
 * them. Secrets (set via `wrangler secret put`) are typed here; if a required
 * secret is missing at runtime the relevant factory fails fast with a clear
 * message rather than erroring at first use.
 */
export interface Env {
  ...
  // Vars — AI Gateway / OpenRouter config for the live LLM path.
  AI_GATEWAY_ACCOUNT_ID?: string;
  AI_GATEWAY_NAME?: string;
  OPENROUTER_MODEL?: string;

  // Secrets
  /** OpenRouter API key. Required only when LLM_PROVIDER=openrouter (opt-in live). */
  OPENROUTER_API_KEY?: string;
  /** Shared secret authorising ingestion requests into the KB pipeline. */
