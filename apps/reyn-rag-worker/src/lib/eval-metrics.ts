 ...
 *
 * - `expectedUrls` empty → returns 1 (trivially satisfied: nothing was expected
 *   and nothing was required, so "all" expectations are met).
 ...
 *
 * - **precision**: fraction of cited URLs that are in `expectedUrls` (how many
 *   cited sources were actually relevant). 0 citations → precision 0. Empty
 *   expected AND 0 citations → precision 0 (degenerate; caller should skip or
 ...
 * information-retrieval convention: an empty query is vacuously "fully recalled"
 * but a system returning no results has zero precision.
 */
export function citationScores(
  ...
  if (cited.size === 0) {
    // precision is undefined with no citations; use 0. recall depends on expected.
 ...
 * requires a live LLM judge (e.g. a second call asking "is this claim supported
 * by the context?"). Use this proxy as a coarse guard only; invest in an LLM
 * judge once the evaluation harness is running against a live worker.
 */
 ...
 * - `p50LatencyMs` and `p95LatencyMs` are the 50th and 95th percentile latencies
 *   computed via nearest-rank on the sorted latency array.
