 ...
 * matches and the cited pages. Per the locked decision the answer quality is
 * summarised as top-K similarity + coverage above a threshold, plus a recency
 * (freshness) term so stale corpora are visibly de-weighted. Every function
 * returns a value in [0, 1] and is 0 for empty input.
 */
...
/**
 * Mean of the top-K cosine match scores. The caller passes the (already
 * top-K-truncated) match scores, so this is the arithmetic mean of them.
 * Returns 0 for an empty list. Cosine scores are assumed in [0, 1]; the result
 * is clamped defensively so the contract holds even if a provider returns a
 * slightly out-of-range score.
 */
